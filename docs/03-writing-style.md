...
开头不要提前展开完整设计。具体代码、扩展需求和机制细节放到“问题”或“最小实现”里。开头只需要让读者知道：当前已经有什么能力，继续往前会卡在哪里。
...
新机制要从具体场景长出来。先写当前做法怎样写，继续这样写会把哪个模块弄乱，再说明这一章把哪段责任移出去。不要先给机制命名，再回头找理由。
...
新接口第一次出现前，先说明它解决哪个痛点、谁调用它、它把数据交给谁。否则接口会像是凭空出现的 API reference。
...
图前后要有文字解释。告诉读者图里的框是什么、箭头表示什么、看完图应该得出什么结论。图里的箭头、标签和框不能互相遮挡，尤其不能让数据流看起来穿过不相关模块。
...
- 每段都用同一种句式开头。
- 结尾只写“为后续打下基础”。
- 作者视角的抽象词，比如“出口”“消费者”“能力体系”。如果必须使用，先绑定具体对象：哪个 event、file、command、output。
