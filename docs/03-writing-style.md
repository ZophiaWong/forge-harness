...
`解决方案` 和 `最小实现` 要分清抽象层级。`解决方案` 讲读者应该形成的 mental model：旧机制怎样卡住、这一章新增哪条分支、结果怎样回到 loop。它可以出现读者可见的核心开关，比如 `runInBackground: true`，但不要在这里展开内部 module、event、callback、wire shape 或文件路径。这些实现细节留到 `最小实现`。
...
如果 `最小实现` 拆成 3 个以上机制或小节，或跨多个模块，进入代码前先放一张导览表。推荐列是 `小节`、`要解决的问题`、`机制`。表格只解释每一块为什么存在，不要把完整 API 和文件清单提前塞进去。
...
先决定图在当前章节承担什么任务。放在 `解决方案` 的图解释方案思路和读者心智模型；放在 `最小实现` 的图解释实现顺序、数据流或状态变化。不要用实现细节图替代方案图，也不要用概念图替代代码流说明。

机制图优先画本章新增的局部路径，不要把后续所有机制都堆进一张不断变大的总图。总图可以单独存在；章节图更像显微镜，只解释这一章新增的那段。

如果一张图同时出现已有机制和本章新增机制，必须用泳道、标签或颜色区分。颜色只能作为辅助，不能成为唯一差异；读者在黑白预览或色弱环境里也应该能看出 old path 和 new path。

图前后要有文字解释。告诉读者图里的框是什么、箭头表示什么、看完图应该得出什么结论。图里的箭头、标签和框不能互相遮挡，尤其不能让数据流看起来穿过不相关模块。

关键流程要用流向表达，不要只写在方框说明里。比如“立即返回”“稍后回流”“分叉后继续执行”这类动作，应该画成箭头、时间线节点或 sequence step，让读者能看出控制流或数据流怎样移动。
...
如果一个机制跨 round、跨 session step，或结果会延迟回流，优先使用 timeline、sequence diagram 或 swimlane 说明顺序。异步机制最容易让读者混淆“已经启动”“已经完成”和“已经被模型看见”，图里要把这些时刻分开。
...
“显式”“默认”“可选”“自动”“只执行一次”这类词第一次出现时，要绑定具体字段、值、触发条件或次数边界。不要只写“模型显式选择后台执行”，要写成“模型在 `bash` tool call 里把 `runInBackground` 设为 `true`”。不要只写“启动结果”，要说清楚它是不是最终结果，比如 `background_started` 只表示 task 已经启动，不表示后台命令已经完成。
...
- 解决方案是否逐项回应了问题。
- `解决方案` 是否停留在概念层和核心开关，没有混入内部实现细节。
- 复杂 `最小实现` 是否先解释每个小节要解决的问题，再进入代码。
...
- 图是否区分已有机制和本章新增机制，关键流程是否通过流向表达。
- 异步、跨 round 或延迟回流机制是否有时间线、sequence diagram 或 swimlane 说明顺序。
- 高风险误解是否在正文第一次出现时就消解，并在末尾 FAQ 或“易混淆的点”里汇总。
