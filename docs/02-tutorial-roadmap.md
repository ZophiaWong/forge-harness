...
`c17` 分成三个 checkpoint。`c17a Shared Team Task Graph` 先让 parent、sync child 和 async child 共享 dependency、status 与 acceptance/evidence；`c17b Long-Lived Teammates / Mailbox` 把一次性 child 扩展成由 Leader 管理的独立 teammate processes；[`c17c Coordination / Completion Protocol`](tutorial/c17c-coordination-completion-protocol.md) 再加入 owner、assign、claim、plan approval、verification、Git integration、shutdown 和 team completion。
...
| [`c17b` Long-Lived Teammates / Mailbox](tutorial/c17b-long-lived-teammates-mailbox.md) | `c15a/c15b Child Sessions` + `c17a Shared Team Task Graph` | fresh child 交出一次 handoff 就结束，不能被持续寻址，也不能等待后续消息。 | Leader 能启动和管理独立 teammate processes，并通过持久 mailbox 做 direct message、fan-out broadcast 和 explicit rejoin。 |
| [`c17c` Coordination / Completion Protocol](tutorial/c17c-coordination-completion-protocol.md) | `c08 Verification / Recovery` + `c14 Worktree Isolation` + `c16a/c16b Extensions` + `c17a/c17b Team Foundations` | task graph 和 mailbox 只能保存状态、传递消息，不能决定谁接任务、谁验证结果、哪些产物可整合，以及团队何时真正完成。 | 一个 capstone run 串起 Leader assign、idle teammate 对 ready + unowned task 的原子 claim、plan approval、verification、Git integration receipt、graceful shutdown 和 completion gate。 |
...
| [`c17b` Long-Lived Teammates / Mailbox](tutorial/c17b-long-lived-teammates-mailbox.md) | `L5 + L3 + L4` | 一次性 child 不能长期存在，也没有可寻址的异步通信路径。 | Leader-managed teammate process、persistent teammate definition、lifecycle、file mailbox、direct/broadcast、explicit rejoin。 | named teammates 能跨多个 turn 存活、收发消息并在失败后由 Leader 重新加入。 |
| [`c17c` Coordination / Completion Protocol](tutorial/c17c-coordination-completion-protocol.md) | all | 有共享工作图和 mailbox，仍缺少 ownership、任务验证、代码整合与团队收尾规则。 | owner/Leader assign、ready-task atomic claim、plan approval、completion evidence、source fingerprint、verification、Git integration receipt、shutdown、completion gate。 | comprehensive run 串起 one-shot children、long-lived teammates、Leader extensions、verification 和已整合产物。 |
