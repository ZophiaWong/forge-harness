...
成员失败时，manager 只会把仍处于 `in_progress` 的 owned task 标记为 `blocked`。如果成员已经成功执行 `submit_result`，task 会保持 `submitted`；即使成员随后因轮次耗尽而没有返回最终文本，Leader 仍可审核这份持久化结果。
...
这条闭环需要 root 创建和分配 task、审核结果、验证并集成 edit，最后再关闭 teammate。若模型每轮只调用一个工具，较小的轮次上限会在协议完成前耗尽。因此本章把 `DEFAULT_MAX_TOOL_ROUNDS` 调整为 48。同步 child 使用 `maxToolRounds=6`；两名长期 teammate 使用 `maxToolRounds=8`，为额外的协调调用、一次失败重试和最终响应保留空间。`maxToolRounds` 统计模型轮次，最终响应也会占用一轮。

```bash
npm run start -- --worktree --verify "npm run build && test -f c17c-coordination-demo.txt && grep -Fx 'issue: FH-16' c17c-coordination-demo.txt && grep -Fx 'status: integrated by c17c' c17c-coordination-demo.txt" '/issue-workflow:triage Run the c17c capstone with one tool call per round. Do not use todo, duplicate messages, or polling calls unless a tool fails. Trust the issue-workflow-demo plugin and call mcp_issue-workflow-demo_lookup_issue with issueId="FH-16". Start research teammate protocol-researcher and edit teammate protocol-editor without taskId; both use maxToolRounds=8. Their teammate_start message must only tell them to remain idle until a later Leader message; it must not mention or act on a task. Create exactly three tasks with dependencies=[]: task_001 is a research task for one synchronous research child, task_002 is a research task for protocol-researcher, and task_003 is an edit task titled="Create c17c coordination artifact" with verificationCommand="grep -Fx '\''issue: FH-16'\'' c17c-coordination-demo.txt && grep -Fx '\''status: integrated by c17c'\'' c17c-coordination-demo.txt". Only after both teammates exist, assign task_001 with task_transition action="assign" and assignee="leader", then assign task_002 with task_transition action="assign" and assignee="protocol-researcher". Do not assign task_003 as Leader. Before delegating, use one separate message_send to tell protocol-researcher about task_002 with the exact lookup result and require exactly task_get, task_add_evidence with external reference "issue-workflow-demo:FH-16", task_transition submit_result, then a final response; do not grep the repository. Also use one separate message_send to tell protocol-editor only to claim task_003, task_get it, submit_plan, then return a final response without writing. Delegate task_001 synchronously with its taskId and maxToolRounds=6. Do not include a skill invocation in the child task. Tell the child that the trusted lookup returned issueId FH-16, title "Plugin components need one loading boundary", status open, and summary "Skills, hooks, and MCP servers should register through existing Forge subsystems." Require task_add_evidence with external reference "issue-workflow-demo:FH-16", then an immediate concise final summary without inspecting repository files. Do not add evidence as Leader for either research task; if the requested actor did not append its own evidence, report failure instead of compensating. After the child returns, submit_result for task_001 as Leader and review_result with decision="pass" and a non-empty reason. Never submit_result as Leader for task_002 or task_003; their owning teammates must submit them. Review the submitted task_002 result with decision="pass" and a non-empty reason. After protocol-editor is idle and its plan is pending, read task_003 once with task_get and approve it using review_plan with decision="approve" and a non-empty reason. Send one separate message telling protocol-editor to write c17c-coordination-demo.txt containing exactly "issue: FH-16\nstatus: integrated by c17c\n", append artifact evidence, submit_result, and return idle. When its mailbox result says it is idle, read the submitted edit once with task_get, run task_verify with the exact contract command, then task_integrate. Call teammate_shutdown with mode="shutdown" for each idle teammate. Return final only after the completion gate is ready.'
```
...
- `teammate_start.message` 会在启动后立即执行，因此初始消息只要求待命；task 创建和分配完成后，Leader 再通过 `message_send` 下发具体工作；
- task prompt 明确写出 Leader ownership、ready task、research evidence 和 edit plan approval 的先后顺序，每一步仍由模型发出实际 function call。

若沿用仓库内的示例配置，启动时还会询问 standalone `demo` MCP 和可选的 `review-helper` plugin；它们不参与本章闭环，可以拒绝。需要批准的是 `issue-workflow-demo` plugin、edit teammate start、editor 的 `write`，以及 `task_verify` / `task_integrate`。批准后，root transcript 应出现真实模型与工具记录，例如：
