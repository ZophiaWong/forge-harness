...
| `L4 State, Evidence & Reliability` | Session metadata, append-only Trace events, RuntimeState projection, verification results, task evidence, integration receipts, offline regression reports, and release evidence bundles. | [`src/runtime/session.ts`](../src/runtime/session.ts), [`src/runtime/trace.ts`](../src/runtime/trace.ts), [`src/runtime/state.ts`](../src/runtime/state.ts), [`src/runtime/verification.ts`](../src/runtime/verification.ts), [`src/runtime/evidenceBundle.ts`](../src/runtime/evidenceBundle.ts), [`src/eval/`](../src/eval) |
...
Raw attempt evidence stays local under `.forge/evals/`. The public summary, JSON report, Markdown report, and promoted baseline reject prompt text, model output, raw tool arguments, absolute paths, and Trace payloads. A historical first valid comparable batch for its then-current identity is recorded in the [offline eval regression report](assets/evidence/offline-eval-regression-report.md); later contract changes create a new identity. See [Offline eval and regression reports](offline-eval.md) for the operating rules.

## Release evidence boundary

[`src/runtime/evidenceBundle.ts`](../src/runtime/evidenceBundle.ts) is the small public seam for shared release evidence. Its internal Runtime modules separately own intent/source identity, atomic run reservations, archive capture, schema safety, and release promotion. Live and Eval adapters contribute raw roots plus domain verdicts; they do not implement their own archive writers. The capture path verifies clean source identity before and after capture, rejects unsafe paths, copies original bytes, writes a per-file private inventory, creates and self-verifies the archive, and only then atomically publishes the run directory.

[`src/portfolio/liveEvidence.ts`](../src/portfolio/liveEvidence.ts) injects capture immediately before the subject Live runner's fixture cleanup and takes final tests, source bytes, and portable Git evidence from the integrated root Session worktree. [`src/eval/releaseEvidence.ts`](../src/eval/releaseEvidence.ts) seals the complete subject Eval root and closes every summary reference; a candidate must name a verified external baseline from the same intent. [`src/cli/evidence.ts`](../src/cli/evidence.ts) exposes preregistration, Live/Eval collection, promotion, and verification as one operator workflow. The run ledger also resolves a failed capture across compatible intents when a collector fix changes only the collector identity. It keeps the existing `--retry-of <run-id>` interface and atomically claims the original run before admitting the sole retry.

Behavioral verdict, capture status, infrastructure validity, baseline eligibility, and promotion eligibility remain separate facts. Public manifests contain source/environment identity, sanitized results, limitations, and hashes. Private archives retain raw Sessions/attempts and require maintainer-controlled storage. This is an integrity and traceability layer, not signer attestation or a secret-scanning system. See the [Release evidence runbook](release-evidence.md).
