...
[`src/portfolio/liveEvidence.ts`](../src/portfolio/liveEvidence.ts) injects capture immediately before the subject Live runner's fixture cleanup. [`src/eval/releaseEvidence.ts`](../src/eval/releaseEvidence.ts) seals the complete subject Eval root and closes every summary reference; a candidate must name a verified external baseline from the same intent. [`src/cli/evidence.ts`](../src/cli/evidence.ts) exposes preregistration, Live/Eval collection, promotion, and verification as one operator workflow.
