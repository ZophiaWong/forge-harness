...
The prompt fixes the walkthrough topology at one edit task and one synchronous edit child. That constraint keeps the interview within a predictable time window and gives the submission, verification, and Git receipt one clear source. It asks the model to keep implementation changes in `src/**` and states the evidence → submission → verification → integration phase order. After the child returns, evidence is recorded before the result is submitted with the returned `childSessionId`; `npm test` verification follows submission, and integration follows a passed verification. Task wording, files to inspect, implementation, edit count, concrete tool choice, and recovery from a safely rejected call remain model-directed.

Before reporting PASS, the validator requires exactly one final successful task verification. Earlier failed verification attempts are tolerated only when the structured Trace records a healthy `invalid_input`, proving that Runtime rejected the request before the verifier ran. A real verifier failure, source drift, malformed approval evidence, or any verification call after the final success still fails the Live run. The validator also reconciles the persisted submission and Git receipt with the final root Worktree. It requires a clean Worktree, binds receipt `targetBefore` to the recorded Worktree base, matches root `HEAD` to `integratedCommit`, and requires the complete `targetBefore..HEAD` path set to equal the child submission's `changedFiles`. The fixed topology is a demo boundary, not a Runtime limit.
