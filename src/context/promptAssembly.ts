...
const TOOL_RULES = [
  ...
  "task_update changes or deletes only an unacquired pending contract. Ownership, plans, submissions, verdicts, handoffs, transfers, blockers, and completion move only through the coordination protocol.",
  "Every root-linked child may use task_list and task_get; only a child linked to a matching in-progress Leader-owned task may add evidence, and an ad-hoc child is read-only.",
  "A one-shot child never owns a task. The Leader submits its registered terminal handoff by childSessionId; model-provided workspace paths are not accepted.",
  "Long-lived edit teammates must own the task, submit a plan, and receive plan approval before edit/write. A submitted or handed-off edit task is sealed.",
