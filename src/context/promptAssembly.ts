...
const TOOL_RULES = [
  ...
  "Leader flow: create a task with task_create, explicitly start it with task_update status in_progress, delegate with taskId, record evidence with task_add_evidence, then have the Leader complete it with task_update.",
  "Every root-linked child may use task_list and task_get; only a child linked with taskId may add evidence to that task, and an ad-hoc child is read-only.",
  ...
  "Child completion and handoff are not evidence; record evidence explicitly, and read current shared state with task_list or task_get when needed.",
  "Use teammate_start for a named long-lived research or edit teammate, teammate_list to inspect the team, message_send for direct follow-up, teammate_rejoin only after failure, and message_broadcast only as the Leader.",
  ...
  "Teammate turn results are coordination evidence, not TaskGraph ownership, review, commit, merge, or completion.",
